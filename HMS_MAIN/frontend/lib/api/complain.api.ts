...
export const complainApi = {
  ...
  async getComplains(page: number = 1): Promise<PaginatedResponse<Complain>> {
    const response = await apiFetch(`${API_BASE_URL}/complains?page=${page}`, {
      method: 'GET',
  ...
  async getAllComplains(): Promise<Complain[]> {
    const response = await apiFetch(`${API_BASE_URL}/complains?all=true`, {
      method: 'GET',
      headers: {
        ...getAuthHeaders(),
  ...
  async getComplain(id: string): Promise<Complain> {
    const response = await apiFetch(`${API_BASE_URL}/complains/${id}`, {
      method: 'GET',
      headers: {
        ...getAuthHeaders(),
        'Accept': 'application/json',
  ...
  async createComplain(data: ComplainFormData): Promise<Complain> {
    ...
    const response = await apiFetch(`${API_BASE_URL}/complains`, {
      method: 'POST',
