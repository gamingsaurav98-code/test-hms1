...
export const blockApi = {
  ...
  async getBlocks(page: number = 1): Promise<PaginatedResponse<Block>> {
    const response = await apiFetch(`${API_BASE_URL}/blocks?page=${page}`, {
      method: 'GET',
  ...
  async getBlock(id: string): Promise<Block> {
    const response = await apiFetch(`${API_BASE_URL}/blocks/${id}`, {
      method: 'GET',
      headers: getAuthHeaders(),
    });
  ...
  async createBlock(data: BlockFormData): Promise<Block> {
    ...
    const response = await apiFetch(`${API_BASE_URL}/blocks`, {
      method: 'POST',
