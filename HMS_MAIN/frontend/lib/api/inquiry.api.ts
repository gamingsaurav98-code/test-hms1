...
export const inquiryApi = {
  ...
  async getInquiries(page: number = 1): Promise<PaginatedResponse<Inquiry>> {
    const response = await apiFetch(`${API_BASE_URL}/inquiries?page=${page}`, {
      method: 'GET',
      headers: {
        ...getAuthHeaders(),
  ...
  async getInquiriesByBlock(blockId: string, page: number = 1): Promise<PaginatedResponse<Inquiry>> {
    const response = await apiFetch(`${API_BASE_URL}/inquiries/block/${blockId}?page=${page}`, {
      method: 'GET',
      headers: {
        ...getAuthHeaders(),
        'Accept': 'application/json',
  ...
  async getInquiry(id: string): Promise<Inquiry> {
    ...
    const response = await apiFetch(`${API_BASE_URL}/inquiries/${id}`, {
      method: 'GET',
      headers: {
        ...getAuthHeaders(),
        'Accept': 'application/json',
  ...
  async deleteAttachment(inquiryId: string, attachmentId: string): Promise<void> {
    const response = await apiFetch(`${API_BASE_URL}/inquiries/${inquiryId}/attachments/${attachmentId}`, {
      method: 'DELETE',
      headers: {
        ...getAuthHeaders(),
        'Accept': 'application/json',
