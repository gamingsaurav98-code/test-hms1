...
export const incomeTypeApi = {
  ...
  async getIncomeTypes(page: number = 1): Promise<PaginatedResponse<IncomeType>> {
    const response = await apiFetch(`${API_BASE_URL}/income-types?page=${page}`, {
      method: 'GET',
  ...
  async getIncomeType(id: string): Promise<IncomeType> {
    const response = await apiFetch(`${API_BASE_URL}/income-types/${id}`, {
      method: 'GET',
      headers: getAuthHeaders(),
    });
  ...
  async createIncomeType(data: IncomeTypeFormData): Promise<IncomeType> {
    const response = await apiFetch(`${API_BASE_URL}/income-types`, {
      method: 'POST',
      headers: {
        ...getAuthHeaders(),
