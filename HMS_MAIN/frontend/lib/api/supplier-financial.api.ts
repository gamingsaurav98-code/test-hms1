...
export const supplierFinancialApi = {
  ...
  async getFinancialsBySupplier(supplierId: string): Promise<SupplierFinancial[]> {
    const response = await apiFetch(`${API_BASE_URL}/suppliers/${supplierId}/financials`, {
      method: 'GET',
      headers: {
        ...getAuthHeaders(),
  ...
  async getFinancial(id: string): Promise<SupplierFinancial> {
    const response = await apiFetch(`${API_BASE_URL}/supplier-financials/${id}`, {
      method: 'GET',
      headers: {
        ...getAuthHeaders(),
        'Accept': 'application/json',
  ...
  async createFinancial(data: SupplierFinancialFormData): Promise<SupplierFinancial> {
    const response = await apiFetch(`${API_BASE_URL}/supplier-financials`, {
      method: 'POST',
      headers: {
        ...getAuthHeaders(),
        'Accept': 'application/json',
