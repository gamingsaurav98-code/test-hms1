...
export const chatApi = {
  ...
  async getComplaintChats(complainId: number): Promise<ChatResponse> {
    const response = await apiFetch(`${API_BASE_URL}/chats/complaint/${complainId}`, {
      method: 'GET',
  ...
  async sendMessage(data: SendMessageRequest): Promise<ChatMessage> {
    const response = await apiFetch(`${API_BASE_URL}/chats/send`, {
      method: 'POST',
      headers: getAuthHeaders(),
  ...
  async editMessage(chatId: number, data: EditMessageRequest): Promise<ChatMessage> {
    const response = await apiFetch(`${API_BASE_URL}/chats/${chatId}/edit`, {
      method: 'PUT',
      headers: getAuthHeaders(),
      body: JSON.stringify(data),
    });
  ...
  async markAsRead(data: MarkAsReadRequest): Promise<{ marked_count: number }> {
    const response = await apiFetch(`${API_BASE_URL}/chats/mark-read`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify(data),
    });
  ...
  async getUnreadCount(data: UnreadCountRequest): Promise<{ unread_count: number }> {
    ...
    const response = await apiFetch(`${API_BASE_URL}/chats/unread-count?${params.toString()}`, {
      method: 'GET',
      headers: getAuthHeaders(),
    });
  ...
  async uploadAttachment(file: File): Promise<{ path: string; filename: string }> {
    ...
    const response = await apiFetch(`${API_BASE_URL}/chats/upload-attachment`, {
      method: 'POST',
      headers: {
        ...getAuthHeaders(),
